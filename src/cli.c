...
void handle_add_new_entry(struct sqlite3* db, const struct config cfg,
    ...
    if (credential_data.id == -1 &&
        std_input("associated e-mail", OPTIONAL_PROMPT, credential_data.email,
...
void handle_retrieve_creddata(struct sqlite3* db, const struct config cfg,
    ...
    if (secure_input("master password", "", master_password, INPUT_BUFF_SIZE) !=
        0) {
    ...
    if (result_count == 0) {
        ...
        if (decrypt_string(key, results[0].iv, results[0].pswd,
                           &results[0].pswd) != 0) {
        ...
        for (int i = 0; i < result_count; i++) {
            if (decrypt_string(key, results[i].iv, results[i].pswd,
                               &results[i].pswd) != 0) {
                fprintf(stderr,
    ...
    // if (retrieve_and_decipher_by_source(db, source, key, &results_str,
    // &result_count) == 0) {
