...
void handle_new_master_pswd(struct sqlite3* db, user_t* user) {
    ...
    */
    ...
    binary_array_t master_key = {0}, new_master_key = {0}, session_key = {0}, secure_buffer = {0};
    unsigned char session_iv[IV_SIZE];
    char new_db_path[INPUT_BUFF_SIZE * 2] = {0}, old_db_path[INPUT_BUFF_SIZE * 2] = {0};
    ...
    */
    ...
    if (encrypt_data(session_key.ptr, session_iv, master_key,
                     &master_key)
        != 0) {
        binary_array_secure_free(&session_key);
    ...
    */
    ...
    if (status_code < 0) {
        if (errno == EEXIST) {
            printf("Previous operation of changing master password ended unexpetedly.\n");
        } else {
    ...
    */
    ...
    if (strcmp(user->username, new_user.username) != 0) {
        strncpy(new_user.username, user->username, sizeof(new_user.username) - 1);
    ...
    */
    ...
    if (decrypt_data(session_key.ptr, session_iv, master_key, &master_key) != 0) {
            binary_array_secure_free(&master_key);
   ...
   /*
    * Decipher with old pswd, set relevant info for new DB, cipher with new and write to new DB
    */
    ...
    if (rename(cfg.db_path, old_db_path) != 0) {
        fprintf(stderr, "rename('%s' -> '%s') failed: %s\n",
                cfg.db_path, old_db_path, strerror(errno));
    }

    if (rename(new_db_path, cfg.db_path) != 0) {
        fprintf(stderr, "rename('%s' -> '%s') failed: %s\n",
                new_db_path, cfg.db_path, strerror(errno));
    }
